...
//Hiện class error và bỏ class allow (css)
...
//Xóa class error và hiện class allow (css)
...
const usernameValidate = () => {
    ...
    if (username === "") {
        ...
        if (checkname == true) {
            ...
            showErrorStyles(usernameContainer, usernameInputBox);
            usernameMessage.innerHTML = "Tài khoản không được chứa kí tự đặc biệt";
        }
    }
}
...
const dateValidate = () => {
    const dateContainer = document.querySelector(".sign-up-date");
    const dateInputBox = document.querySelector(".input-date");
    const dateMessage = document.querySelector(".date-message");
    const dateword = dateInputBox.value;

    if (dateword == "") {
        showErrorStyles(dateContainer, dateInputBox)
        dateMessage.innerHTML = "Hãy nhập ngày sinh";
    }
    else {
        removeErrorStyles(dateContainer, dateInputBox)
...
const phoneValidate = () => {
    ...
    const phoneword = phoneInputBox.value;

    if (phoneword === "") {
        showErrorStyles(phoneContainer, phoneInputBox)
        phoneMessage.innerHTML = "Hãy nhập số điện thoại";
    }
    else if (phoneword.length < 10 || phoneword.length > 10) {
        showErrorStyles(phoneContainer, phoneInputBox)
        phoneMessage.innerHTML = "Số điện thoại không hợp lệ";
    }
    else {
        var number_phone = Number(phoneword)
        if (Number.isNaN(number_phone)) {
            showErrorStyles(phoneContainer, phoneInputBox);
            phoneMessage.innerHTML = "Số điện thoại phải là số";
        }
        else {
...
const addEvent = (() => {
    ...
    inputBoxes[6].addEventListener("blur", () => {
        repasswordValidate();
    });
    /* add sự kiện */
    ...
    submitButton.addEventListener("click", e => {
        ...
        if (errorMessage) {
            e.preventDefault();  //chặn ko cho click
        }
    });
})();